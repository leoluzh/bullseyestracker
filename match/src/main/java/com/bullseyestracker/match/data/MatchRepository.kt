...
class MatchRepository(private val dao: MatchDao) {
    ...
    fun observeInProgressMatch(): Flow<Match?> =
        dao.observeInProgressMatch().map { entity -> entity?.let { assembleMatch(it) } }

    fun observeCompletedMatches(): Flow<List<Match>> =
        dao.observeCompletedMatches().map { entities -> entities.map { assembleMatch(it) } }

    suspend fun createMatch(gameMode: GameMode, playerNames: List<String>): Match {
        require(playerNames.size in 2..4) { "A match requires 2-4 players (spec FR-007, SC-004)." }
        val matchId = UUID.randomUUID().toString()
        val playerEntities = playerNames.mapIndexed { index, name ->
            Player(
